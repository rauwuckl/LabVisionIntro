...
int main (int argc, char *argv[]){
	...
	std::string experimentShortName;
	std::cin >> experimentShortName;
	...
	string experimentName = get_time_string() + string("_") + experimentShortName + string("_") + string(to_string(input_epochs)) + string("_epochs");
	...
	int test_network_every_n_epochs = 5;
	bool human_readable_storage = false;

	bool save_spikes_while_training = true;
	...
	if (starting_epoch != 1){
		std::string weightlocation;
		weightlocation = output_folder_path + experimentName + "/" + "training/" + "epoch" + string(to_string(starting_epoch)) + "/" + "Synapses_NetworkWeights.bin";
	...
	std::string destination = output_folder_path + experimentName+"/"+modelname;
